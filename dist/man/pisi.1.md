...
pisi - LupuS package manager

# SYNOPSIS

`pisi [options] <command> [arguments]`

# DESCRIPTION

`pisi` is the package manager for the LupuS operating system. It is used to
manage installed software packages, search for available software and to apply
updates to the system.

# OPTIONS

The following options are applicable to `pisi(1)`.
 ...
 * `-h`, `--help`
   ...
   Print the command line options for `pisi(1)` and exit.

 * `--version`

   Print the `pisi(1)` version and exit.

 * `-N`, `--no-color`

   Disable the use of ANSI escape sequences for colourisation by pisi.

# SUBCOMMANDS

All available subcommands are listed below by their primary name and their
alias, if available. Most commands in pisi support a short form.

`add-repo (ar) <repo-name> <repo URI>`

    Add a new repository to the system with the given name and URI. Note
    that a valid pisi index file will start with `pisi-index.xml` and
...
`blame (bl) <packagename>`
    ...
    changelog. This will integrate automatically with `lupbuild(1)`
    git changelog support for official LupuS packages, and allow
...
`build (bi) <path to pspec.xml>`
    ...
    Consult `pisi ? bi` for further details. The legacy `pisi` format
    is no longer supported by LupuS and is only currently used behind
    the scenes in the third party mechanism. New packages should only
    use `package.yml(5)` via `ypkg(1)` and `lupbuild(1)`
...
`clean`
    ...
    of pisi. This should only be used if the package manager refuses
...
`delete-cache (dc)`
    ...
    Clear out any temporary caches still held by `pisi` for downloads
...
`delta (dt) <oldpackage1> <newpackage>`
    ...
    for manual repo management you can use this command. A `.delta.pisi`
 ...
 * `-O`, `--output-dir`:
        ...
        Override the output directory for the `.delta.pisi`
        instead of using the current working directory.

 * `-F`, `--package-format`:

        Override the pisi internal format. Expert option only,
 ...
 * `-o`, `--output-dir`:
        ...
        Override the output directory for the `.pisi` instead of
...
`history (hs)`
    ...
    Manage the pisi transaction history. Every operation via `pisi`
    ...
    due to the rolling nature of LupuS, and that old packages may
...
`index (ix) <directory>`
    ...
    Produce an `pisi-index` repository in the given directory
...
`install (it) <name>`
    ...
    Install a named package or local `.pisi` directly onto the system.
 ...
 * `-b`, `--with-build-host`:
        ...
        Packages built by LupuS' official build server come from the
        build host "lupus". By default, packages built locally come
...
`rebuild-db (rdb)`
    ...
    Rebuild all `pisi` databases. This may be required if pisi
...
`remove-orphans (rmo)`
    ...
    Note that in LupuS terminology an orphan is a proveable
...
# EXAMPLES
 ...
 1. Install a package and its dependencies:
    ...
    ```bash
    sudo pisi install nano
    ```
 
 2. Uninstall a package and orphaned dependencies:

    ```bash
    sudo pisi remove-orphans nano
    ```
...
`usysconf(1)`, `lupbuild(1)`, `ferryd(1)`, `ypkg(1)`, `package.yml(5)`

 * https://help.teknoanka.com/docs/packaging
