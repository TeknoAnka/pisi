...
"""
    ...
    def setup_options(self):
        ...
        group.add_option(
            ...
            action="store_true",
            default=False,
            help=_("Security related package upgrades only"),
        )
        group.add_option(
            "-b",
            "--bypass-update-repo",
            action="store_true",
            default=False,
            help=_("Do not update repositories"),
        )
        group.add_option(
        ...
        group.add_option(
            ...
            action="append",
            default=None,
            help=_("Upgrade component's and recursive components' packages"),
        )
        group.add_option(
            "-r",
            "--repository",
            ...
            type="string",
            default=None,
            help=_("Name of the to be upgraded packages' repository"),
        )
        group.add_option(
            "-f",
            "--fetch-only",
        ...
        if components:
            ...
            for name in components:
                if componentdb.has_component(name):
                    if repository:
                        packages.extend(
                            componentdb.get_packages(name, walk=True, repo=repository)
                        )
                    else:
