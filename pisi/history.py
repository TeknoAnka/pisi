...
class Package(metaclass=autoxml.autoxml):
    ...
    def __str__(self):
        ...
        if self.operation == "upgrade":
            if self.type == "delta":
                return _("%s is upgraded from %s to %s with delta.") % (
                    self.name,
                    self.before,
                    self.after,
                )
            else:
                return _("%s is upgraded from %s to %s.") % (
                    self.name,
                    self.before,
                    self.after,
                )
            ...
            return _("%s is downgraded from %s to %s.") % (
                self.name,
                self.before,
                self.after,
            )
...
class History(xmlfile.XmlFile, metaclass=autoxml.autoxml):
    ...
    def add(self, pkgBefore=None, pkgAfter=None, operation=None, otype=None):
        ...
        for histInfo, pkgInfo in [
            (package.before, pkgBefore),
            (package.after, pkgAfter),
        ]:
            if pkgInfo:
