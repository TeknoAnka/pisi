...
def is_method_missing(exception):
    ...
    if exception._dbus_error_name in (
        "tr.org.pardus.comar.python.missing",
...
def post_install(
    ...
    for script in provided_scripts:
        ...
        script_name = safe_script_name(script.name) if script.name else package_name
        if script.om == "System.Package":
            self_post = True
        try:
            link.register(
                script_name, script.om, os.path.join(scriptpath, script.script)
            )
    ...
    if self_post:
        ...
        try:
            link.System.Package[package_name].postInstall(
                fromVersion, fromRelease, toVersion, toRelease, timeout=ctx.dbus_timeout
            )
        except dbus.DBusException as exception:
