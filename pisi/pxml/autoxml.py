        ...
        if not nodes:
            if self.req == MANDATORY:
                errs.append(
                    where
                    + ": "
                    + _("At least one '{}' tag should have local text.").format(
                        self.tag
                    )
                )
        else:
...
class autoxml(oo.autosuper, oo.autoprop):
    ...
    and (not implemented yet): xmlfile.Bpisi
        ...
        if xmlfile_support:
            ...
            def write(
                ...
                if errs:
                    ...
                    raise Error(*errs)
                self.writexml(
                    uri, tmpDir, sha1sum=sha1sum, compress=compress, sign=sign
                )
                if not keepDoc:
                    self.unlink()  # get rid of the tree
    ...
    def gen_tag(cls, tag, spec):
        ...
        if isinstance(tag_type, type) and tag_type in autoxml.basic_cons_map:
            ...
            raise Error(
                _("gen_tag: unrecognized tag type {} in spec.").format(str(tag_type))
            )
    ...
    def parse_spec(cls, token, spec):
        ...
        if len(spec) >= 3:
            ...
            if isinstance(token_type[0], autoxml):
                ...
                # if list of ordpisi type, just take the name for
                path = token
        ...
        def errors(value, where):
            ...
            if value and not isinstance(value, token_type):
                errs.append(
                    where
                    + ": "
                    + _("Type mismatch. Expected {0}, got {1}").format(
