    ...
    for fileName in filePathGlob:
        if can_access_file(fileName):
            try:
                ...
                ctx.ui.error(
                    _("ActionsAPI [chmod]: Operation not permitted: %s (mode: 0%o)")
                    % (fileName, mode)
                )
        else:
...
def chown(filePath, uid="root", gid="root"):
    ...
    if can_access_file(filePath):
        try:
            ...
            ctx.ui.error(
                _("ActionsAPI [chown]: Operation not permitted: %s (uid: %s, gid: %s)")
                % (filePath, uid, gid)
            )
    else:
...
def sym(source, destination):
    ...
    try:
        ...
        ctx.ui.error(
            _("ActionsAPI [sym]: Permission denied: %s to %s") % (source, destination)
        )
...
def unlink(pattern):
    ...
    if len(filePathGlob) == 0:
        ctx.ui.error(
            _('No file matched pattern "%s". Remove operation failed.') % pattern
        )
        return
    ...
    for filePath in filePathGlob:
        if isFile(filePath) or isLink(filePath):
            try:
                ...
                ctx.ui.error(
                    _("ActionsAPI [unlink]: Permission denied: %s.") % (filePath)
                )
...
def copytree(source, destination, sym=True):
    ...
    if isDirectory(source):
        if os.path.exists(destination):
            if isDirectory(destination):
                copytree(
                    source, join_path(destination, os.path.basename(source.strip("/")))
                )
                return
            else:
