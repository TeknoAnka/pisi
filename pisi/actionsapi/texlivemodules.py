    ...
    for formatfile in ls("%s/texmf/fmtutil/format*.cnf" % get.curDIR()):
        ...
        for line in symfile.readlines():
            if not line.startswith("#"):
                ...
                if (
                    "cont-" in symbin[0]
                    or "metafun" in symbin[0]
                    or "mptopdf" in symbin[0]
                ):
                    ...
                    if symbin[0] == symbin[1]:
                        ctx.ui.info(
                            _("Symlink %s -> %s skipped.") % (symbin[0], symbin[1])
                        )
                    elif can_access_file(
                        "%s/usr/bin/%s" % (get.installDIR(), symbin[0])
                    ):
                        ctx.ui.info(_("Symlink %s skipped (file exists.)") % symbin[0])
                    else:
                        ctx.ui.info(
                            _("Making symlink from %s to %s") % (symbin[0], symbin[1])
                        )
                        dodir("/usr/bin")
    ...
    for installdoc in ["texmf", "texmf-dist", "tlpkg", "texmf-var"]:
        if os.path.isdir("%s/%s" % (get.curDIR(), installdoc)):
            if not installdoc == "texmf-var":
                shutil.copytree(
                    installdoc, "%s/usr/share/%s" % (get.installDIR(), installdoc)
                )
            else:
            ...
            for configFile in files:
                if configFile.endswith(("cfg", "cnf")):
                    ...
                    if not os.path.isdir(
                        "%s/etc/texmf/%s.d" % (get.installDIR(), dirname)
                    ):
                        ctx.ui.info(_("Creating /etc/texmf/%s.d") % dirname)
                        dodir("/etc/texmf/%s.d" % dirname)
                    ctx.ui.info(
                        _(
                            "Moving (and symlinking) /usr/share/texmf/%s to /etc/texmf/%s.d"
                        )
                        % (configFile, dirname)
                    )
                    domove(
                        "/usr/share/texmf/%s/%s" % (dirname, configFile),
                        "/etc/texmf/%s.d" % dirname,
                    )
                    dosym(
                        "/etc/texmf/%s.d/%s" % (dirname, configFile),
                        "/usr/share/texmf/%s/%s" % (dirname, configFile),
                    )
...
def addFormat(parameters):
    ...
    if not os.path.isfile(
        "%s/texmf/fmtutil/format.%s.cnf" % (get.curDIR(), get.srcNAME())
    ):
        cnf_file = open(
            "%s/texmf/fmtutil/format.%s.cnf" % (get.curDIR(), get.srcNAME()), "w"
        )
        cnf_file.write(
            "# Generated for %s by actionsapi/texlivemodules.py\n" % get.srcNAME()
        )
        cnf_file.close()
    ...
    cnf_file = open(
        "%s/texmf/fmtutil/format.%s.cnf" % (get.curDIR(), get.srcNAME()), "a"
    )
    cnf_file.write(
        "%s\t%s\t%s\t%s\n"
        % (
...
def buildFormatFiles():
    ...
    if os.path.isdir("%s/texmf/fmtutil/" % get.curDIR()):
        for formatfile in ls("%s/texmf/fmtutil/format*.cnf" % get.curDIR()):
            ...
            export(
                "TEXMFHOME",
                "%s/texmf:/%stexmf-dist:%s/texmf-var"
                % (get.curDIR(), get.curDIR(), get.curDIR()),
            )
            export("VARTEXFONTS", "fonts")
            system(
                "env -u TEXINPUTS fmtutil --cnffile %s --fmtdir texmf-var/web2c --all"
                % formatfile
            )
    ...
    for tlpobjfile in ls("%s/tlpkg/tlpobj/*" % get.curDIR()):
        ...
        for line in jobsfile.readlines():
            ...
            if splitline[0] == "execute":
                ...
                if command == "addMap":
                    echo(
                        "%s/%s.cfg" % (get.curDIR(), get.srcNAME()),
                        "Map %s" % parameter,
                    )
                    ctx.ui.info(
                        _("Map %s is added to %s/%s.cfg")
                        % (parameter, get.curDIR(), get.srcNAME())
                    )
                elif command == "addMixedMap":
                    echo(
                        "%s/%s.cfg" % (get.curDIR(), get.srcNAME()),
                        "MixedMap %s" % parameter,
                    )
                    ctx.ui.info(
                        _("MixedMap %s is added to %s/%s.cfg")
                        % (parameter, get.curDIR(), get.srcNAME())
                    )
                elif command == "addDvipsMap":
                    echo(
                        "%s/%s-config.ps" % (get.curDIR(), get.srcNAME()),
                        "p +%s" % parameter,
                    )
                    ctx.ui.info(
                        _("p +%s is added to %s/%s-config.ps")
                        % (parameter, get.curDIR(), get.srcNAME())
                    )
                elif command == "addDvipdfmMap":
                    echo(
                        "%s/%s-config" % (get.curDIR(), get.srcNAME()),
                        "f %s" % parameter,
                    )
                    ctx.ui.info(
                        _("f %s is added to %s/%s-config")
                        % (parameter, get.curDIR(), get.srcNAME())
                    )
                    ...
                    ctx.ui.info(
                        _("No rule to proccess %s. Please file a bug.") % command
                    )
        jobsfile.close()
