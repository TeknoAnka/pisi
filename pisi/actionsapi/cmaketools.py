...
def configure(
    parameters="", installPrefix="/%s" % get.defaultprefixDIR(), sourceDir="."
):
...
def rawInstall(parameters="", argument="install"):
    ...
    if (
        can_access_file("makefile")
        or can_access_file("Makefile")
        or can_access_file("GNUmakefile")
    ):
